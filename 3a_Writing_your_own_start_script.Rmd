...
If you navigate to **scripts/start/default.R**, you'll see taht the **default.R** script has the following scturcture:
...
This tells the R environment to source **start_functions.R** from the **scripts** folder. Due to time constraints, we won't look into what start_functions.R does but we just have to remember that this script loads the conifg file and then loads start_run function with this config which is needed to start MAgPIE runs.
...
As **magpie_workshop.R** is just a renamed version of **default.R**, the contents are the same and we'll now start making changes to this **magpie_workshop.R** script. Users are now requested to open this **magpie_workshop.R** script in R or in a text editor of their choice. The contents should look like this:
...
Remember that the above line just loads the object called **cfg** in R environment. This contents of this **cfg** object can be found in Table 1 from tutorial 3 (Changing MAgPIE configuration).
...
Lets add some basic settings before moving to the main changes. These changes would be made to the **cfg** object and we'll use the notation **cfg$\<some_setting\>** for this purpose (Ref. Table 1 from tutorial 3 for detials). 
...
You'll see that we had already changed these settings by hand in tutorial 3. Here, these settings will be changed in **deafult config** using the **magpie_workshop.R** starting script which should look like this now:
